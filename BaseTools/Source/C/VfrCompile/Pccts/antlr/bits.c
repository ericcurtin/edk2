...
#endif
{
	...
	if ( name!=NULL ) {
		fprintf(ErrFile, "SetWordType %s%s[%ld] = {",
				name,
                suffix,
...
#endif
{
	...
	if ( name!=NULL ) {
		fprintf(Parser_h, "\tstatic SetWordType %s%s[%ld];\n", name, suffix,
				NumWords(TokenNum-1)*sizeof(unsigned));
		fprintf(Parser_c, "SetWordType %s::%s%s[%ld] = {",
				CurrentClassName,
				name,
				suffix,
				NumWords(TokenNum-1)*sizeof(unsigned));
...
#endif
{
  ...
  if ( FirstAction != NULL ) dumpAction(FirstAction,Parser_c, 0, -1, 0, 1);    /* MR11 MR15b */
	...
	/* Build constructors */
	fprintf(Parser_c, "\n%s::", CurrentClassName);
	fprintf(Parser_c,	"%s(ANTLRTokenBuffer *input) : %s(input,%d,%d,%d,%ld)\n",
						CurrentClassName,
